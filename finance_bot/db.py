...
class FinanceDB:
    ...
    def add_transaction(
        ...
        with self.connect() as conn:
            cursor = conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (user_id, type_, float(amount), clean_category, note or "", timestamp.isoformat(timespec="seconds")),
            )
        ...
        with self.connect() as conn:
            cursor = conn.execute(
                ...
                INSERT INTO subscriptions
                    (user_id, name, amount, next_payment_date, remind_7, remind_3, remind_1, active)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    user_id,
    ...
    def list_active_subscriptions(self, user_id: int) -> list[sqlite3.Row]:
        with self.connect() as conn:
            return conn.execute(
                """
                SELECT id, user_id, name, amount, next_payment_date, remind_7, remind_3, remind_1, active
                FROM subscriptions
    ...
    def find_subscription(self, user_id: int, value: str) -> sqlite3.Row | None:
        ...
        with self.connect() as conn:
            if value.isdigit():
                return conn.execute(
                    """
                    SELECT id, user_id, name, amount, next_payment_date, active
                    FROM subscriptions
            ...
            return conn.execute(
                ...
                SELECT id, user_id, name, amount, next_payment_date, active
                FROM subscriptions
                WHERE user_id = ? AND LOWER(name) = LOWER(?) AND active = 1
    ...
    def due_subscriptions(
        ...
        field_by_day = {1: "remind_1", 3: "remind_3", 7: "remind_7"}
        field = field_by_day[days_ahead]
        target_date = (today or datetime.now(APP_TZ).date()).toordinal() + days_ahead
        target = date.fromordinal(target_date).isoformat()
    ...
    def mark_subscription_paid(self, subscription_id: int, user_id: int | None = None) -> sqlite3.Row | None:
        with self.connect() as conn:
            ...
            row = conn.execute(
                ...
                SELECT id, user_id, name, amount, next_payment_date
                FROM subscriptions
                WHERE id = ? {user_clause} AND active = 1
                """,
                params,
            ).fetchone()
            if not row:
            ...
            conn.execute(
                ...
                (next_date.isoformat(), subscription_id),
            )
            return conn.execute(
                """
                SELECT id, user_id, name, amount, next_payment_date
    ...
    def upsert_budget(self, user_id: int, category: str, limit_amount: float, month: str | None = None) -> int:
        ...
        with self.connect() as conn:
            conn.execute(
                ...
                """,
                (user_id, clean_category, float(limit_amount), budget_month),
            )
            row = conn.execute(
                """
                SELECT id
                ...
                """,
                (user_id, clean_category, budget_month),
            ).fetchone()
            return int(row["id"])
    ...
    def get_budget(self, user_id: int, category: str, month: str | None = None) -> sqlite3.Row | None:
        with self.connect() as conn:
            return conn.execute(
                ...
                WHERE user_id = ? AND category = ? AND month = ?
                """,
                (user_id, normalize_category(category), month or current_month()),
            ).fetchone()
