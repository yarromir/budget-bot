...
- умеет подключать Nous Hermes API для более свободного текста;
...
# Опционально: LLM-парсер свободных фраз через Nous Hermes API
export NOUS_API_KEY="nous_api_key"
export NOUS_MODEL="Hermes-4-70B"
# export NOUS_BASE_URL="https://inference-api.nousresearch.com/v1/chat/completions"
# export NOUS_TIMEOUT_SECONDS="8"
```
...
- добавить подписку и выбрать напоминания за 7, 3 или 1 день до оплаты;
...
/clear transactions yes
/clear subscriptions yes
/clear budgets yes
/clear all yes
```
...
Если обычный парсер не понял сообщение, а задан `NOUS_API_KEY`, бот отправит текст в Nous Hermes API и попробует распознать свободную фразу вроде `вчера в кафе оставил 1200` без команды.
...
Команда `/balance` или фраза `баланс` показывает доходы, расходы и остаток за всё время. Команда `/last` или фраза `последние операции` выводит последние 10 операций прямо в чате. Команды `/clear transactions yes`, `/clear subscriptions yes`, `/clear budgets yes` и `/clear all yes` очищают только данные текущего пользователя; без `yes` бот покажет запрос подтверждения.
